...
name: deep-research
description: |-
  深度调研方法论（8步法）：将模糊主题转化为高质量调研报告。系统化执行问题拆解、资料分层(L1-L4)、事实抽取、对比框架、推导验证，输出可交付的结构化报告。Use proactively when user wants in-depth research, comparative analysis, or investigation reports.
  
  Examples:
  - user: "深度调研 REST API 和 GraphQL 的区别" → execute 8-step methodology
  - user: "帮我调研 AI Agent 的发展趋势" → time-sensitive analysis with 6-month window
  - user: "对比分析 PostgreSQL vs MongoDB" → decision support framework
  - user: "写一份关于 XXX 的调研报告" → deliver structured report with source verification
---
